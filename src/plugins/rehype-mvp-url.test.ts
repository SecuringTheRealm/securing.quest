...
describe('processHref', () => {
	it('adds WT.mc_id to a bare Microsoft URL', () => {
		...
		const mcId = url.searchParams.get('WT.mc_id');
		expect(mcId).not.toBeNull();
		expect(MVP_IDS).toContain(mcId);
	});

	it('replaces existing WT.mc_id', () => {
		const result = processHref('https://learn.microsoft.com/path?WT.mc_id=AI-MVP-5004204');
		const url = new URL(result);
	...
	it('replaces lowercase wt.mc_id (case-insensitive)', () => {
		const result = processHref('https://learn.microsoft.com/path?wt.mc_id=OLD_VALUE');
		const url = new URL(result);
	...
	it('preserves other query params while adding WT.mc_id', () => {
		const result = processHref('https://learn.microsoft.com/path?tab=foo');
		const url = new URL(result);
	...
	it('preserves other params and replaces WT.mc_id when both exist', () => {
		const result = processHref('https://learn.microsoft.com/path?tab=foo&WT.mc_id=old');
		const url = new URL(result);
		expect(url.searchParams.get('tab')).toBe('foo');
		const mcId = url.searchParams.get('WT.mc_id');
		expect(mcId).not.toBeNull();
...
describe('distribution', () => {
	it('assigns both MVP IDs across 100+ distinct paths with ≥30% each', () => {
		...
		for (let i = 0; i < totalPaths; i++) {
			const result = processHref(`https://learn.microsoft.com/path-${i}/page`);
			const url = new URL(result);
